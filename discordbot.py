...
async def on_reaction_add(reaction, user):
    if reaction.count == 2:
        if reaction.emoji == "\N{DIGIT ZERO}\N{COMBINING ENCLOSING KEYCAP}":
            ...
            del dice[1]
            await reaction.message.channel.send(f'```内訳：1D100 {dice} {user.name.mention}```')
        if reaction.emoji == "\N{DIGIT ONE}\N{COMBINING ENCLOSING KEYCAP}":
            dice = diceroll(1, 10)
            await reaction.message.channel.send(f'➨ **{dice[1]}**')
            del dice[1]
            await reaction.message.channel.send(f'```内訳：1D10 {dice} {user.name.mention}```')
        if reaction.emoji == "\N{DIGIT TWO}\N{COMBINING ENCLOSING KEYCAP}":
            dice = diceroll(3, 6)
            await reaction.message.channel.send(f'➨ **{dice[3]}**')
            del dice[3]
            await reaction.message.channel.send(f'```内訳：3D6 {dice} {user.name.mention}```')
        if reaction.emoji == "\N{DIGIT THREE}\N{COMBINING ENCLOSING KEYCAP}":
            dice = diceroll(1, 6)
            await reaction.message.channel.send(f'➨ **{dice[1]}**')
            del dice[1]
            await reaction.message.channel.send(f'```内訳：1D6 {dice} {user.name.mention}```')
        if reaction.emoji == "\N{DIGIT FOUR}\N{COMBINING ENCLOSING KEYCAP}":
            dice = diceroll(1, 4)
            await reaction.message.channel.send(f'➨ **{dice[1]}**')
            del dice[1]
            await reaction.message.channel.send(f'```内訳：1D4 {dice} {user.name.mention}```')  
        if reaction.emoji == "\N{DIGIT FIVE}\N{COMBINING ENCLOSING KEYCAP}":
            dice = diceroll(2, 3)
            await reaction.message.channel.send(f'➨ **{dice[2]}**')
            del dice[2]
            await reaction.message.channel.send(f'```内訳：2D3 {dice} {user.name.mention}```')
